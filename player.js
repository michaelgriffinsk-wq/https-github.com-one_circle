 ...
 * Upgraded with Spotify-style Shuffle, Loop States, and Audio Output Device Picker
 */
(function() {
    ...
    playerContainer.innerHTML = `
        <style>
          ...
          .player-minimized {
            ...
            padding: 10px 16px; height: 72px; max-width: 1400px; margin: 0 auto;
          }
          .player-maximized {
          ...
          .player-maximized.active { transform: translateY(0); }
        ...
        <div id="persistentPlayer">
           <!-- MINIMIZED BAR -->
           <div class="player-minimized" id="expandTrigger" style="cursor: pointer;">
              <div style="display: flex; align-items: center; gap: 12px; min-width: 0;">
                 ...
                 <div style="min-width: 0;">
                    <h4 id="miniTitle" style="margin: 0; font-size: 14px; font-weight: 700; color: #f5efe5; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">No Track Playing</h4>
                    <p id="miniArtist" style="margin: 2px 0 0 0; font-size: 12px; color: #aaa398; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">One Circle</p>
                 </div>
              </div>
           ...
           <div class="player-maximized" id="maximizedPlayer" style="display: none;">
              ...
              <div id="swipeArea" style="display: flex; flex-direction: column; align-items: center; width: 100%; max-width: 400px; margin: auto; text-align: center;">
                 ...
                 <div style="width: 100%; text-align: left; margin-bottom: 20px;">
                    <h2 id="maxTitle" style="margin: 0; font-size: 24px; font-weight: 700; color: #f5efe5; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">Select a track</h2>
                    <p id="maxArtist" style="margin: 6px 0 0 0; font-size: 15px; color: #aaa398; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">One Circle</p>
                 </div>
    ...
    class GlobalPersistentPlayer {
        ...
        initListeners() {
            ...
            this.audio.addEventListener('play', () => { this.isPlaying = true; this.updatePlayStateUI(); });
            this.audio.addEventListener('pause', () => { this.isPlaying = false; this.updatePlayStateUI(); });
        ...
        setExpanded(expand) {
            ...
            if (expand) {
                maxEl.style.display = 'flex';
                setTimeout(() => maxEl.classList.add('active'), 10);
            } else {
                maxEl.classList.remove('active');
                setTimeout(() => { if (!this.isExpanded) maxEl.style.display = 'none'; }, 400);
            }
        }
        ...
        playTrack(track, playlist = [], index = 0) {
            ...
            document.getElementById('miniArtist').textContent = track.artist || "One Circle";
            document.getElementById('miniImg').src = track.image || '';

            document.getElementById('maxTitle').textContent = track.title;
            document.getElementById('maxArtist').textContent = track.artist || "One Circle";
            document.getElementById('maxImg').src = track.image || '';

            // Handle audio source (supports simulated preview tones or valid file URLs)
            if (track.file) {
        ...
        updatePlayStateUI() {
            const playSvg = this.isPlaying 
                ? '<polygon points="6 4 10 4 10 20 6 20 6 4"></polygon><polygon points="14 4 18 4 18 20 14 20 14 4"></polygon>' 
                : '<polygon points="5 3 19 12 5 21 5 3"></polygon>';
            
            document.getElementById('miniPlayIcon').innerHTML = playSvg;
            document.getElementById('maxPlayIcon').innerHTML = playSvg;
        }
        ...
        generateShuffleQueue() {
            ...
            // Ensure current track is first in shuffled sequence
        ...
        toggleLoop() {
            this.loopMode = (this.loopMode + 1) % 3; // 0: Off, 1: Loop All, 2: Loop One
            const btn = document.getElementById('loopBtn');
            if (this.loopMode === 0) {
                btn.classList.remove('active');
                btn.innerHTML = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="17 1 21 5 17 9"></polyline><path d="M3 11V9a4 4 0 0 1 4-4h14"></path><polyline points="7 23 3 19 7 15"></polyline><path d="M21 13v2a4 4 0 0 1-4 4H3"></path></svg>';
            } else if (this.loopMode === 1) {
                btn.classList.add('active');
                btn.innerHTML = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="17 1 21 5 17 9"></polyline><path d="M3 11V9a4 4 0 0 1 4-4h14"></path><polyline points="7 23 3 19 7 15"></polyline><path d="M21 13v2a4 4 0 0 1-4 4H3"></path></svg>';
            } else if (this.loopMode === 2) {
                btn.classList.add('active');
                // Spotify-style Loop One indicator (icon with small numeral '1')
        ...
        saveState() {
            const state = { currentTrack: this.currentTrack, currentIndex: this.currentIndex, playlist: this.playlist };
            ...
            if (saved) {
                try {
                    ...
                    if (data.currentTrack) {
                        ...
                        document.getElementById('miniTitle').textContent = this.currentTrack.title;
                        document.getElementById('miniArtist').textContent = this.currentTrack.artist || "One Circle";
                        document.getElementById('miniImg').src = this.currentTrack.image || '';

                        document.getElementById('maxTitle').textContent = this.currentTrack.title;
                        document.getElementById('maxArtist').textContent = this.currentTrack.artist || "One Circle";
                        document.getElementById('maxImg').src = this.currentTrack.image || '';
